...
namespace SolomonCopy.Upgrade
{
    public class LevelUpController : MonoBehaviour
    {
        ...
        public void OpenChoice(int playerLevel)
        {
            ...
            int finalChoiceCount = choicesPerLevel;
            if (MetaProgressionManager.Instance != null)
                finalChoiceCount += MetaProgressionManager.Instance.GetLevelUpChoicesBonus();
            int maxUiChoices = (cardButtons != null && cardButtons.Length > 0) ? cardButtons.Length : 3;
            finalChoiceCount = Mathf.Clamp(finalChoiceCount, 1, maxUiChoices);

            var picks = pool.RandomPick(finalChoiceCount, playerLevel, applier.StackCount);
