...
namespace SolomonCopy.Player
{
    public class MagicCaster : MonoBehaviour
    {
        ...
        private void FireBase(BaseMagicSO m, Vector2 dir)
        {
            ...
            if (p != null)
                p.Initialize(dir, m.speed, m.lifetime, m.damage, m.pierce, m.statusEffect, m.statusDuration);
            _cooldownUntil = Time.time + m.cooldown;
        ...
        private void FireCombo(ComboMagicSO c, Vector2 dir)
        {
            ...
            var p = go.GetComponent<Projectile>();
            if (p != null)
                p.Initialize(dir, c.speed, c.lifetime, c.damage, c.pierce, c.statusEffect, c.statusDuration,
                             c.explodeOnHit, c.aoeRadius, c.chainCount, c.chainRange);
            _cooldownUntil = Time.time + c.cooldown;
